...
testClean$day_nmr <- day_nmr
testClean$month <- month

  #Remove date 
test_time_add <- testClean %>% 
  select(c(sentiment, cleanTweets, day, day_nmr, month, hour, minutes, at_dummy))

#Encode data
tweet_encode_test <- as.data.frame(LFmodel$encode(testClean$cleanTweets))

#Bind columns 
tweet_encode_bind <- cbind(test_time_add,tweet_encode)
tweet_encode_bind$sentiment <- factor(tweet_encode_bind$sentiment)
  #Drop cleanTweets
...
blueprint_tweet_sentiment <- recipe(x = tweet_encode_recipeReady,
  ...
  step_num2factor(sentinment,
                  transform = function(x) x + 1,
                  levels = c('Negative', 'Neutral', 'Positive')) %>% 
  step_normalize(all_numeric_predictors())

recipe <- blueprint_tweet_sentiment %>% prep() %>% bake()
...
#Hyperparameter grid 
grid <- data.frame(cp=seq(0,0.01,.001))
```

#GLM
```{r}
set.seed(0294875)
model_glmnet <- caret::train(sentiment~., data = tweet_encode_recipeReady,
                          method= "glmnet",
                          trainControl = cv)

prediction <- predict(model_rpart, test_encode, type = 'prob')
prediction
```
